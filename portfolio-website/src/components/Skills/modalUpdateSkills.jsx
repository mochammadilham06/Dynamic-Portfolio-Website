...
import UseUpdateSkills from "../../configs/Hooks/Operation/mutationUpdateSkills";

const ModalUpdateSkills = ({ data }) => {
  ...
  const { updateSkillsData, updateSkillsError, updateSkillsLoading } =
    UseUpdateSkills();
  ...
  const handleUpload = (imageUpdate) => {
    if (!imageUpdate) {
      ...
      const imageName = ref(storage, `skills/${imageUpdate.name}`);
  ...
  if (updateSkillsLoading) {
    return <LoadingSvg />;
  }

  if (updateSkillsError) {
    return <h1>`${updateSkillsError.message}`</h1>;
  }
...
export default ModalUpdateSkills;
